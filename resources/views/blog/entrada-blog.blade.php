    ...
    <div class="row">
        <div id="cuerpo_blog" class="col-md-10" style="padding: 20px; margin: -50px auto 0; align-content:center;background-color: white; box-shadow: 0px 4px 5px 0px rgba(161,161,161,1);">
            ...
            <div class="col-md-12">
                ...
                @if($autor->id != 999)
                    <div>Escrito por <b style="color:#1d3b4f;">{{$autor->nombre}}&nbsp;{{$autor->apellido_p}}</b>, {{$fecha}} &middot; No. de Visita: {{$entrada->visitas}}</div>
                @else
                    <div>Escrito por <b style="color:#1d3b4f;">{{$entrada->autor_externo}}</b>, {{$fecha}} &middot; No. de Visita: {{$entrada->visitas}}</div>
                @endif
                <br><br>
